...
def search(filename, threshold_limits = (210, 255)):
    ...
    while True:
        ...
        def threshold(img, limits=threshold_limits):
        ...
        cv2.imwrite('debug.jpg', img); #sys.exit()
        ...
        if len(contours) > 15: #04
            ...
            print('Contour not good (%i), new threshold limits %s !' % (len(contours), str(threshold_limits)))
            continue
        ...
        for cnt in contours:
            ...
            return egg_count, original

        break

    return 0, original

egg_count, img = search(filename)
